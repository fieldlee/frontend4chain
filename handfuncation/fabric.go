...
func addOrg(writer http.ResponseWriter, request *http.Request) {
	...
	for _, ip := range iplist {
		...
		restartsh := fmt.Sprint("chmod +x ./restart.sh;", "./restart.sh;")
	...
	config.Port = "4000"
	config.CurOrgId = ""
	config.KeyValueStore = "/tmp/fabric-client-kvs"
	...
	for _, org := range project.Orgs {
		...
		// /tmp/certification/f18fafe1e2b4494696e1dac580ab6c53/crypto-config/peerOrganizations/nxia.jiake.com/users/Admin@nxia.jiake.com/msp/keystore
